...
then
    ...
    docker run --name some-wordpress -v /var/lib/mysql:/var/lib/mysql --link some-mysql:mysql -p 80:80 -d wordpress:3.9.2
...
docker run --name some-wordpress --link some-mysql:mysql -p 80:80 -d wordpress:3.9.2
EOF
