...
The work in this project reported development of a novel, lightweight, robust and reliable proxy that supports dynamic software upgrade with no downtime and automatic rollback in response to the application failure. The proxy has the ability to manage application clusters using Docker as a container virtualization while being reliable, and stable under heavy load.
...
\noindent \\
\textbf{Implementation Chapter} - Presents the implementation approaches used to develop the dynamic software update proxy. In addition, this chapter explains different components of the proxy and their functionality in detail. \hl{Finally, it explains how to install and run the proxy. }  
...
\subsection*{Download and Install Go}
...
Docker can be installed from \url{https://docs.docker.com/installation/#installation}. After Docker installation to ensure that the Docker daemon listens on a TCP socket and the proxy can connect to it the following two commands should be executed: \medskip
