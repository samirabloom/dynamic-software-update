...
\subsection{Load Balancing Feature}
...
The proxy was configured to forward HTTP requests to the three nodes in Couchbase cluster (Figure \ref{couchbase_cluster}) using round-robin loading balancing.   The proxy echos back the start-up configuration as shown in the following terminal output.\bigskip
\begin{lstlisting}[language=terminal]
...
When the proxy is configured to run with INFO level logging it prints log entries to the terminal that demonstrate the HTTP requests are being distributed between each node in the cluster using round-robin order.\smallskip
...
\subsection{Automated Software Update} 
To prove the proxy is capable of coping with dynamic software updates and it is suitable for contentious delivery, INSTANT, SESSION, GRADUAL and CONCURRENT updates were performed while the proxy was running.  As it was mentioned in section \ref{Quantitative_Analysis} WordPress currently runs approximately 20\% of all Web sites, therefore it was chosen for evaluating the dynamic software update feature of the proxy. Different update scenarios were tested on version 3.9.1 and 3.9.2 of WorlPress. Each version of WorlPress application was run within a Docker container inside a virtual machine as demonstrated in section \ref{load}. 

\subsubsection*{INSTANT Update}
The following steps were followed for evaluation of the proxy using INSTANT mode (Section \ref{modes}).\medskip

\noindent
\textit{Proxy Configuration} - the proxy was configured to forward HTTP requests to the cluster running version 3.9.1 of the WordPress application. The start-up configuration echoed by proxy is shown in the following terminal output.\smallskip
...
\noindent\\
When the proxy was run in the INFO level logging, its output to the terminal demonstrated that the HTTP requests were forwarded to the version 3.9.1 of the WordPress.\smallskip
...
\noindent\\
This was further confirmed by the response received from the WordPress application via proxy. Note that the "X-Powered-By" and "X-Pingback" headers refer to the PHP version and the netwrok address of version 3.9.1 of WordPress application respectively. Note that cookie associated with a cluster with INSTANT upgrade mode does not have an expiry date. \smallskip  
...
\noindent\\
\textit{PUT Request For INSTANT Upgrade} - To upgrade the WordPress application from version 3.9.1. to version 3.9.2 instantly, a cluster containing new version was added to the proxy using the following PUT request: \smallskip 
...
The following steps were followed for evaluation of the proxy using SESSION mode (Section \ref{modes}).\medskip

\noindent
\textit{Proxy Configuration} - The proxy was configured with a cluster running version 3.9.1 of the WordPress application. The start-up configuration echoed by proxy is shown in the following terminal output.\smallskip
...
\noindent \\
\textit{HTTP Requests \& Responses} - HTTP requests were sent to the WordPress cluster via proxy suing the following cURL syntax:\smallskip  
...
\noindent\\
\textit{PUT Request For SESSION Upgrade} - A cluster running the version 3.9.2 of the WordPress was added to the proxy with SESSION upgrade transition mode and 60 seconds session time out using the following PUT request: \smallskip 
...
\noindent\\
The proxy acknowledged addition of the new cluster by outputting the following message in terminal when it runs in the INFO logging level: \smallskip 
...
\noindent\\
The following shows the PUT request and the associated HTTP response containing the cluster id received from the proxy.\smallskip 
...
\noindent\\
\textit{HTTP Requests Without Cookie} - To check the proxy behaves correctly with a SESSION upgrade, a HTTP request with no cookie was sent to the proxy. The proxy forwarded the HTTP request to the new cluster as shown in the following terminal output: \smallskip   
...
\noindent\\
Analyzing the headers in the response (\textit{i.e.} "X-Powered-By" and "X-Pingback") also confirmed that the responses are coming from the new cluster with version 3.9.2. Note that the Set-Cookie header in the response from a cluster with SESSION upgrade mode has an expiry date. \smallskip
...
\noindent\\
\textit{HTTP Requests With Cookie} - After the SESSION upgrade when the HTTP requests were sent to the proxy with a uuid cookie associated with a cluster with older version, the proxy forwarded the request to the old cluster rather than the new version as it is shown in the following proxy output printed in the terminal and the response received by the client. \medskip 

\noindent
\textit{HTTP request with cookie Using cURL}
\begin{lstlisting}[language=terminal]
curl -v 'http://127.0.0.1:1235/' \
...
\noindent\\
The response received via proxy from the WordPress application after the oldest version failed is shown here. Note that the "X-Pingback" header shows the network address of old version of the WordPress application. In addition, Since the cluster with SESSION upgrade mode has been deleted by proxy the uuid cookie in the "Set-Cookie" header does not have an expiry date.\smallskip 
...
This section summarizes the performance evaluation of the proxy when a GRADUAL upgrade of the WordPress from version 3.9.1 to 3.9.2 was applied using proxy. \medskip

\noindent
\textit{Proxy Configuration} - the initial configuration of the proxy is shown in the following terminal output echoed by proxy when it was started. Note that the proxy was configured with version 3.9.1 of WordPress application.\smallskip
...
\noindent\\
\textit{HTTP Requests \& Responses} - HTTP requests were sent to WordPress application via proxy using the following the cURL syntax:\smallskip
...
\noindent\\
\textit{PUT Request for GRADUAL Upgrade} - The following PUT request was sent to the proxy for upgrading WordPress application from version 3.9.1 to version 3.9.2.
...
\noindent \\
The following output echoed by the proxy showing successful addition of the new cluster with GRADUAL transition mode and version 3.9.2 to the proxy. \smallskip
...
\noindent\\ 
A cluster associated to the version 3.9.2 of the WolrdPress application was returned by proxy.\smallskip
...
\noindent\\
\textit{GRADUAL Upgrade} - HTTP requests with a transition cookie were sent to the proxy after the new cluster was added.\smallskip
...
\noindent\\
\textit{Handling Incorrect Behavior} - To insure the proxy can go back to the older version of the application if the new version behaved incorrectly, the Docker container running version 3.9.2 of the WordPress application was stopped. The behavior of the proxy was screened when new HTTP requests with transition cookie were sent to it. The following terminal output shows that when the proxy fails connecting to the new version of the application, it removes the cluster from the configuration and forwards requests to the previous cluster version. \smallskip
...
\noindent\\
The following response received after the failure of the cluster with version 3.9.2 showed that version 3.9.1 with IP address "192.168.50.30" is processing the requests.\smallskip
...
To evaluate that the CONCURRENT mode works reliably with proxy, WordPress version 3.9.1 was upgraded to version 3.9.2 via proxy. The upgrade process and the proxy's reaction to the upgrade and requests / responses associated with the proxy is summarized in this section.

\noindent
\textit{Proxy Configuration}
