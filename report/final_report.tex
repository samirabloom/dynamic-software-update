...
For both saturation and load tests, requests were sent to WordPress and Couchbase directly and via the proxy. Figure \ref{absoloute-wordpress} and \ref{absoloute-couchbase} show the latency in milliseconds added when sending requests via the proxy in comparison to sending requests directly to either application.  Figure \ref{percentage-wordpress} and \ref{percentage-couchbase} show the latency added when sending requests via the proxy as a percentage of the latency when sending requests directly to the applications. All performance figures show the latency using a percentile distribution; the percentage indicates the proportion of the requests that have the recorded latency or lower. For example, a latency of 40 milliseconds at the 99th percentile indicates that 99\% of the requests have a 40 milliseconds latency or less.   

The evaluation results from WordPress application indicated that in the majority of the time processing requests via proxy was significantly more efficient. It is not fully clear why processing requests via the proxy is faster than using WordPress directly. These findings require further investigation, however, one hypothesis is that the proxy manages connections more efficiently than the WordPress application using Apache HTTP Server. This is because the proxy is written in Go programming language and uses goroutines\footnote{\label{golangtutorials}\url{http://golangtutorials.blogspot.co.uk/2011/06/goroutines.html}}  instead of threads to handle multiple requests where each requests is processed in two goroutines. One goroutine is used for the request and one goroutine is used for the response. Goroutines are executed on thread pools and therefore are not one to one with a thread. This means that when a new connection comes into a proxy generally a new thread will not be created and instead an existing thread will be used from the thread pool. Therefore, the proxy does not close connections as aggressively as Apache HTTP Server and it sends multiple requests down the same connection, which is more efficient, if the client does not close the connection. This is particularly true for HTTP/1.1 which supports connection re-uses when the "Connection" header has a value of "keep-alive".    
...
Moreover, figure xx  and xx shows that the proxy has a greater standard deviation for response times because as the percentile increases up to 99\%, the overhead added by the proxy becomes greater. Go garbage collector and other sporadic activities happening at a low level inside the Go runtime could be an explanation for these findings.

Similar to the WordPress, Couchbase results also showed that processing request via the proxy was mainly more efficient than processing them directly by Couchbase or if any overhead was added by the proxy it was insignificant.

Couchbase performance and saturation tests showed that the proxy add a small over head (<6\%) when it is run for shorter period of time, however, it is significantly increases the performance of the couchbase when it is run for medium and long periods (Figure xx, yy). Although further analysis is required to justify these findings, one hypothesis could be that Couchbase is more efficient at managing request for a short period because it is holding all the connection open. However after a period of time (>10 seconds) it runs out of resources and has to start closing connections more aggressively, therefore, its efficiency at handling connections reduces.
