...
This report presents a novel, lightweight, robust and reliable proxy that supports dynamic software upgrade with no downtime and automatic rollback in response to failure. The main features provided are:
\begin{list}{-}{}
\itemsep0em
\item Seamless dynamic software upgrade using four different upgrade mechanisms:
	\begin{list}{-}{}
		\itemsep0em
		\item{Instant upgrade} - immediate upgrade to the new version of the application 
		\item{Session upgrade} - upgrading new sessions to the updated version  
		\item{Gradual upgrade} - gradual upgrade of new sessions to the updated version
		\item{Concurrent upgrade} - the old and the new versions of the application concurrently process requests  
	\end{list}
...
\item Two mechanisms to manage software versions:
	\begin{list}{-}{}
		...
		\item Distinct Docker images for each version of an application.  The proxy manages a Docker container based on each image and therefore isolating each application version in a separate userspace sand-boxed process providing resource isolation while sharing a single operating system kernel. 
	\end{list}
\item Two mechanisms configure the proxy:
	\begin{list}{-}{}
		\itemsep0em
		\item Configuration file in JSON format which is parsed when the proxy loads. All features of the TCP based server configuration and Docker image based configuration are supported.
		\item A REST API in JSON / HTTP format with can be used to query, add or remove server and cluster configurations.  All features of the TCP based server configuration and Docker image based configuration are supported.
	\end{list}
\item Extensive support for managing Docker containers and images, including support for all features of Docker such as:
	\begin{list}{-}{}
		\itemsep0em
		\item Volume (Directory) Binding
		\item Port & IP Binding
		\item Container Linking
...
\chapter{xxxxx   xxxxx    xxx  Evaluation add one sentence for recovery speed test}
This Chapter demonstrates the quantitative and qualitative analysis performed on the dynamic software update proxy presented in Chapter \ref{Implementation}. The quantitative analysis focused on the load and saturation performance of the proxy and while qualitative analysis focused on the functional correctness and robustness of the proxy. 
...
\section{xxx xxxxx   xxxx Quantitative Analysis add one sentence for recovery speed test}\label{Quantitative_Analysis}
