...
\lstdefinestyle{TerminalInputStyle}{
  language=terminal,
...
%  numbers=left,
  ...
  numbersep=3pt,
  frame=tb,
  frameround =ht|fiht|fiht|fiht|fi 
...
A HTTP request has the following format:\bigskip
...
A HTTP response has the following format:\bigskip
...
\begin{lstlisting}[language=terminal,firstnumber=1]
Usage of proxy:
...
XXXXXXXX A FIGURE FOR TESTING XXXXXXXXXXXXXXXXXXXXXXXXXXXXX\bigskip
...
XXXXXXXX MAYBE ADD A SCREEN SHOT XXXXXXXXXXXXXXXXXXX
...
To test the load balancing feature of the dynamic software update proxy, HTTP requests were sent to a Couchbase cluster running in a Docker container (Figure \ref{couchbase_cluster}). The cluster contained three nodes running Couchbase server 2.5.1. Couchbase servers were chosen for load balancing evaluation since they are designed to be clustered from a single machine to very large scale deployments. The following section summarizes steps executed for load balancing evaluation. 
...
\noindent
\textit{Proxy Configuration} - the proxy was configured to direct HTTP requests to the three nodes in Couchbase cluster (Figure \ref{couchbase_cluster}) as shown in the following terminal output. Since the nodes are running within a Docker container, their IP address has been exposed and therefore, the IP address seen in figure \ref{couchbase_cluster} is different from the one proxy is configured with xxxxxxxx also TALK ABOUT THE 1 HR TIME DIFFERENCE IN THE PROXY WITH WORLDPRESS AND COACHBASE\bigskip
\begin{lstlisting}[language=terminal]
...
\end{lstlisting}
\noindent\\
\textit{HTTP Requests} - simultaneous HTTP requests were sent to the running proxy using cURL as follows:
\begin{lstlisting}[language=terminal]
curl -v http://127.0.0.1:1235
\end{lstlisting}

\noindent\\
\textit{Load balancing proxy} - analysis of the proxy showed that the HTTP requests were successfully distributed to the the cluster of Couchbase servers by proxy in a round-robin order. The following terminal output demonstrates the network address of each server that the proxy has received the response from. 
...
\noindent\\
\textit{HTTP Responses} - the following is an example HTTP response received from servers in the Couchbase cluster via proxy: 
...
To prove the suitability of the proposed system for continuous delivery, application updates will be performed using both a bash script \footnote{\url{http://www.tldp.org/LDP/abs/html/}} and a puppet manifest \footnote{\url{https://puppetlabs.com/}}. Puppet has been chosen since it is the most widely used automated deployment tool. In addition bash has been chosen since all automated deployment tools have the ability to run bash commands. 
