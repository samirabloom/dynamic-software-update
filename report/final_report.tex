...
\noindent\\
This was further confirmed by the response received from the WorldPress application via proxy. Note that the "X-Powered-By" and "X-Pingback" headers refer to the PHP version and the netwrok address of version 3.9.1 of WorldPress application respectively. Note that cookie associated with a cluster with INSTANT upgrade mode does not have an expiry date. \smallskip  
...
\noindent\\
\textit{PUT Request For INSTANT Upgrade} - To upgrade the WorldPress application from version 3.9.1. to version 3.9.2 instantly, a cluster containing new version was added to the proxy using the following PUT request: \smallskip 
...
\noindent
\textit{Proxy Configuration} - The proxy was configured with a cluster running version 3.9.1 of the WorldPress application. The start-up configuration echoed by proxy is shown in the following terminal output.\smallskip
...
\noindent\\
The "X-Powered-By" and "X-Pingback" headers received by the client also confirmed that the response has been sent by WorldPress version 3.9.1.  Note that the Set-Cookie header in the response is set and it has no expiry date. \smallskip 
...
\noindent\\
\textit{PUT Request For SESSION Upgrade} - A cluster running the version 3.9.2 of the WorldPress was added to the proxy with SESSION upgrade transition mode and 60 seconds session time out using the following PUT request: \smallskip 
...
\noindent\\
The response received via proxy from the WorldPress application after the oldest version failed is shown here. Note that the "X-Pingback" header shows the network address of old version of the WorldPress application. In addition, Since the cluster with SESSION upgrade mode has been deleted by proxy the uuid cookie in the "Set-Cookie" header does not have an expiry date.\smallskip 
...
\noindent\\
\textbf{GRADUAL Update}

\noindent
This section summarizes the performance evaluation of the proxy when a GRADUAL upgrade of the WorldPress from version 3.9.1 to 3.9.2 was applied using proxy. \medskip

\noindent
\textit{Proxy Configuration} - the initial configuration of the proxy is shown in the following terminal output echoed by proxy when it was started. Note that the proxy was configured with version 3.9.1 of WorldPress application.\smallskip
...
\noindent\\
\textit{HTTP Requests \& Responses} - HTTP requests were sent to WorldPress application via proxy using the following the cURL syntax:\smallskip
...
\noindent\\
The following proxy output confirms that the HTTP requests were forwarded to the WorldPress application version 3.9.1. Since no upgrade mode was chosen for the cluster, the default mode \textit{i.e.} INSTANT mode was chosen by proxy. \smallskip
...
\noindent\\
"X-Pingback" header of the response received from the proxy further confirmed that proxy is receiving responses from WorldPress application version 3.9.1.\smallskip 
...
\noindent\\
\textit{PUT Request for GRADUAL Upgrade} - The following PUT request was sent to the proxy for upgrading WorldPress application from version 3.9.1 to version 3.9.2.
